...
Momentum.prototype = {
	...
	start : function ( velocity ) {
		...
		//trace ( ' - this.time:', this.time );
		
		this._startTime = Date.now();
		//trace ( ' - this._startTime:', this._startTime );
	...
	_settle : function () {
		...
		if ( time_local >= this.time ){
			...
			//trace ( this.distance, this._prevDis );
			return;
		} else {
			var dis_at_time = ( this.velocityInital * time_local ) + ( .5 * this._accel * time_local * time_local );
			dis_at_time *= this._DIST_MULTI;
			this.displacement = dis_at_time - this._prevDis;
			this._prevDis = dis_at_time;
		}

		//trace ( time_local, velocityCurrent.toFixed(4), dis_at_time.toFixed(4), this.displacement.toFixed(4) );	
		
		this.onUpdate ( this );	
		//trace ( time_local.toFixed(4), dis_at_time.toFixed(4), velocityCurrent.toFixed(4) );
